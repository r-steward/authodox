...
export class MemoryDao<T extends object, ID, SC> {
    ...
    constructor(key: string, predicate: (t: T, sc: SC) => boolean, timeout: number = 0, items?: ReadonlyArray<T>) {
        this._key = key;
    ...
    search(sc: SC): Promise<T[]> {
        return new Promise(resolve => {
            setTimeout(
                () => resolve(Array.from(this.data.values()).filter(i => this._predicate(i, sc))),
                this._timeout
            );
        });
    }
    ...
    newId(): number {
        const ids: any = Array.from(this.data.entries()).map(i => this._getId(i[1]));
